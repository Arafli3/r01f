...
def setup_user():
    ...
    print(f"{Fore.RED}┃{Fore.BLUE}         Create a vmess account       {Fore.RED} ┃")
    ...
    while not exittol:
        ...
        if Quota == "":
            while True:
                ...
                opsi = input(f"\033[93mSelect Y to continue, select N to cancel (Y/N):\033[0m").strip().lower()
                if opsi == "y":
                    Quota = "unlimited"
                    exittol = True
    ...
    if user:
        ...
        for inbound in config['inbounds']:
            if inbound['protocol'] == 'vmess':
                inbound['settings']['clients'].append(vmuser)
                break
        for inbound in config['inbounds']:
            if inbound['protocol'] == 'vmess':
                if inbound['streamSettings']['network'] in ['grpc']:
                    inbound['settings']['clients'].append(vmgrpc)
                    break
        ...
        vmess_tls1 = json.dumps(vmess_Tls, indent=2)
        vmess_ntls2 = json.dumps(vmess_ntls, indent=2)
        vmess_grpc3 = json.dumps(vmess_grpc, indent=2)
        vmess_link1 = "vmess://" + base64.b64encode(vmess_tls1.encode()).decode()
        vmess_link2 = "vmess://" + base64.b64encode(vmess_ntls2.encode()).decode()
        vmess_link3 = "vmess://" + base64.b64encode(vmess_grpc3.encode()).decode()
        css = textwrap.dedent("""\
            {
            margin: 0;
            padding: 0;
...
<body>
  ...
  skip-cert-verify: true
  grpc-opts:
    grpc-service-name: vmess-grpc</textarea>
    ...
    <script>
        ...
        log(f" \033[93malterId        :\033[0m \033[92m0\033[0m ", log_file)
        log(f" \033[93mSecurity       :\033[0m \033[92mauto\033[0m ", log_file)
        log(f" \033[93mNetwork        :\033[0m \033[92mws\033[0m ", log_file)
        log(f" \033[93mPath           :\033[0m \033[92m/vmess\033[0m ", log_file)
        log(f" \033[93mService Name   :\033[0m \033[92mvmess-grpc\033[0m ", log_file)
        log(f" {separator_blue} ", log_file)
        log(f" \033[93mLink TLS       :\033[0m \033[92m{vmess_link1}\033[0m ", log_file)
        log(f" {separator_red} ", log_file)
        log(f" \033[93mLink none TLS  :\033[0m \033[92m{vmess_link2}\033[0m ", log_file)
        log(f" {separator_blue} ", log_file)
        log(f" \033[93mLink gRPC      :\033[0m \033[92m{vmess_link3}\033[0m ", log_file)
        log(f" {separator_red} ", log_file)
        log(f" \033[93mFormat OpenClash :\033[0m \033[92mhttps://{domain}:81/vmess-{user}\033[0m ", log_file)
